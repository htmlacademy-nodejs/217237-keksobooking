...
router.get(``, (req, res) => {
  const skip = checkProperty(req.query, `skip`, SKIP_DEFAULT);
  const limit = checkProperty(req.query, `limit`, LIMIT_DEFAULT);
  const data = offers.slice(skip).slice(0, limit);

  res.send({skip: Number(skip), limit: Number(limit), total: data.length, data});
});
